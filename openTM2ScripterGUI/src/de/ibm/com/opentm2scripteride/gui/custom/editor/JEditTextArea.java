 ...
 *     + "}");</pre>
 *
 * @author Slava Pestov, modified by Stefan Burniki, GeorgVolk, Tim Reppenhagen in 2012
 * @version $Id: JEditTextArea.java,v 1.36 1999/12/13 03:40:30, modified 2011/12/14
 */
...
{	
	...
	public JEditTextArea(TextAreaDefaults defaults)
	{
		...
		mAppInitialized = false;
		
		// Initialize some misc. stuff
		painter = new AdvancedTextAreaPainter(this,defaults);
		...
		addMouseWheelListener(new MouseWheelListener(){
			...
			{
				vertical.setValue(vertical.getValue()+e.getWheelRotation()*e.getScrollAmount());
			}
	...
	protected boolean mAppInitialized;
	
	protected AdvancedTextAreaPainter painter;
	protected JScrollPane scrollPane;
	protected EditorPopupMenu popup;
	...
	protected boolean verticalAdded;
	protected boolean horizontalAdded;
	...
	protected long mDocSavedTime = 0L;
	protected long mDocChangedTime = 0L;
	protected UndoManager mUndomanager = null; 
	
	/**
	 * Should be called if everything is initialized and ready for use
	 */
	 ...
	 */
	public final AdvancedTextAreaPainter getPainter()
	{
		return painter;
	}
	...
	public void updateScrollBars()
	{
		...
		if(vertical != null) {
			...
			if (visibleLines < totalLines && !verticalAdded) {
				add(RIGHT, vertical);
				verticalAdded = true;
				layoutchange = true;
			} else if (visibleLines >= totalLines && verticalAdded) {
				remove(vertical);
				verticalAdded = false;
				layoutchange = true;
			}
			if (visibleLines != 0) {
				vertical.setValues(firstLine,visibleLines,0,totalLines);
		...
		if(horizontal != null)
		{
			int maxWidth = getMaxWidth() + painter.getFontMetrics().charWidth('w');
			if ((horizontalOffset != 0 || maxWidth > width) && !horizontalAdded) {
				add(BOTTOM, horizontal);
				horizontalAdded = true;
				layoutchange = true;
			} else if (horizontalOffset == 0 && maxWidth <= width && horizontalAdded) {
				remove(horizontal);
				horizontalAdded = false;
				layoutchange = true;
			}
			if (width != 0) {
				//+10 is padding
				horizontal.setValues(-horizontalOffset,width,0,maxWidth);
	...
	public boolean scrollTo(int line, int offset)
	{
		...
		int lineNumberWidth = painter.getLineNumberingWidth();
		int x = _offsetToX(line,offset) + lineNumberWidth;
		int width = painter.getFontMetrics().charWidth('w');

		if(x < 0)
		{
			newHorizontalOffset = Math.min(0,horizontalOffset
				- x + lineNumberWidth + width + 5);
		}
	...
	public void setCaretPosition(int line, int offset) {
		...
		Element correctLine = root.getElement(line);
		setCaretPosition(correctLine.getStartOffset() + offset);
	}
	/**
	...
	public void setSelectedText(String selectedText)
	{
		...
		{
			bl.printStackTrace();
			ErrorHandler.getInstance().handleError(1, "Cannot replace"
				+ " selection");
		}
	...
	public void paste()
	{
		if(editable)
		{
			...
			{
				getToolkit().beep();
				ErrorHandler.getInstance().handleError(1, "Clipboard does not contain a String");
			}
		}
	}
	...
	protected int getMaxWidth() {
		...
		for (int i = 0; i < root.getElementCount(); i++) {
			Element cur = root.getElement(i);
			int curlen = cur.getEndOffset() - cur.getStartOffset() - 1;
			if (curlen > max) {
				max = curlen;
				line = i;
			}
		}
		return offsetToX(line, max + 1) - horizontalOffset + painter.getLineNumberingWidth();
	}
	...
	public int[] wordAroundPosition(int offset, String extraNoWordSep, boolean symbolwords) {
		int line = document.getDefaultRootElement().getElementIndex(offset);
		String lineText = getLineText(line);
		if (lineText == null || lineText.isEmpty()) {
			return null;
		}
		int relOffset = offset - document.getDefaultRootElement().getElement(line).getStartOffset();
		char ch = lineText.charAt(Math.max(0,relOffset - 1));
		String noWordSep = (String)document.getProperty("noWordSep");
		noWordSep = (noWordSep == null) ? extraNoWordSep : noWordSep + extraNoWordSep;
		...
		for(int i = relOffset; i < lineText.length(); i++)
		{
			ch = lineText.charAt(i);
			if(selectNoLetter ^ (!Character
				.isLetterOrDigit(ch) &&
		...
		int lineStart = getLineStartOffset(line);
		int[] beginEnd = {lineStart + wordStart,lineStart + wordEnd};
		return beginEnd;
	}
	
	protected void updateCodeCompleter() {
		if (!mAppInitialized) {
			return;
		}
		int caret = getCaretPosition();
		...
		String lineText = getLineText(line);
		//ignore code completer in comments
		if (lineText == null || lineText.isEmpty() || lineText.charAt(0) == '*') {
			//CodeCompleter.getInstance().allcommands();
			MainApp.getInstance().getMainWindow().getCodeCompleteList().setListContent(CodeCompleter.getInstance().getAllCommands());
			return;
		}
		int[] beginEnd = wordAroundPosition(caret, "_", false);
		if (beginEnd == null) {
			//CodeCompleter.getInstance().allcommands();
			MainApp.getInstance().getMainWindow().getCodeCompleteList().setListContent(CodeCompleter.getInstance().getAllCommands());
			return;
		...
		try {
			String word = getDocument().getText(beginEnd[0], caret - beginEnd[0]);
			CodeCompleter.getInstance().setFilter(word, false);
		} catch (Exception e) {}
	}
	...
	class ScrollLayout implements LayoutManager
	{
		...
		public void layoutContainer(Container parent)
		{
			...
			int bottomHeight = (bottom == null) ? 0 : bottom.getPreferredSize().height;
			...
			if (bottom != null) {
				bottom.setBounds( ileft, itop + centerHeight, 
	...
	class DragHandler implements MouseMotionListener
	{
		public void mouseDragged(MouseEvent evt)
		{
			...
			int x = evt.getX() - painter.getLineNumberingWidth();
			select(getMarkPosition(),xyToOffset(x,evt.getY()));
		}
	...
	class MouseHandler extends MouseAdapter
	{
		public void mousePressed(MouseEvent evt)
		{
			...
			{
				popup = new EditorPopupMenu(JEditTextArea.this, line);
				popup.show(painter,evt.getX(),evt.getY());
				return;
			}

			int offset = xToOffset(line,evt.getX() - painter.getLineNumberingWidth());
			...
			int offset, int dot) throws BadLocationException
		{
			...
			int[] beginEnd = wordAroundPosition(offset + startOffset, "", true);
			if (beginEnd != null) {
				select(beginEnd[0], beginEnd[1]);
			}
		}
	...
	public void setDocSavedTime(long docSavedTime) {
		mDocSavedTime = docSavedTime;
	}
	
	public long getDocSavedTime() {
		return mDocSavedTime;
	}
	
	public void setDocChangedTime(long docChangedTime) {
		mDocChangedTime = docChangedTime;
	}
	
	public long getDocChangedTime() {
		return mDocChangedTime;
	}
	
	public boolean isDocSaved() {
		return (mDocChangedTime<=mDocSavedTime);
	}
	...
	public void configUndoManagerLimit(int lv) {
		mUndomanager.setLimit(lv);
	}
	...
	public void addDocUndoableEditListener() {
		document.addUndoableEditListener(mUndomanager);
	}
