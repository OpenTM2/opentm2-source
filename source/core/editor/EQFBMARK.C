...
// GQ: - fixed P023077: Adding dictionary words into translation sometimess not work
// 
// 
...
// GQ: - fixed P02835 EQF0611 message hangs TM
//       by avoiding endless loops in EQFBFuncNextWord
// 
// 
...
// 
// 
// $Revision: 1.3 $ ----------- 5 Apr 2004
// --RJ: P019364: EQFBFindWord: add new param fFindAllWords; use fFindAllWOrds= TRUE if called
// --             from EQFBFuncWordDel, FALSE otherwise
// 
//
// $Revision: 1.2 $ ----------- 16 Feb 2004
// --RJ: P018931: EQFBFuncMarkNextWord: use FuncDoEndLine(pDoc, TRUE) instead of FuncEndLine
//
//
...
//
//
// $Revision: 1.3 $ ----------- 14 Nov 2003
// --RJ: S000007: FuncDelTilTag: if LINEBREAK_CHAR is SOFTLF_CHAR, allow deletion even
//          in reflow-protected markups
//
//
// $Revision: 1.2 $ ----------- 27 Oct 2003
// --RJ: P018311: FuncDelTilTag: if area to be deleted contains a LF, but markup does not allow reflow,
//                 nothing is deleted
//
//
...
//
//
// $Revision: 1.3 $ ----------- 17 Sep 2003
// --RJ: P016169: EQFBFuncWordDel: LF at end of word is not deleted with the word
// --             also punctuation at end of word is not deleted with the word
//
//
// $Revision: 1.2 $ ----------- 14 Jul 2003
// --RJ: P017589: EQFBFuncResetMarkInSeg: delete checking if (!fNoCUASel)
// --RJ: P017589: EQFBFuncMarkDelete: if !EQFBFlags.Reflow: delete allowed if mark has no LF
//
//
...
//
//
// $Revision: 1.4 $ ----------- 22 Apr 2003
// -- RJ: P016967: Corrected overwriting of text (tags included) if working with IME
//
//
...
//
//
// $Revision: 1.3 $ ----------- 29 Oct 2002
// --RJ: IV0068: EQFBFuncCaps+EQFBFuncWordMark: add workaround if pstEQFGen is NULL (e.g. in ITM)
//
//
// $Revision: 1.2 $ ----------- 29 Jul 2002
// --RJ: changes from PUT QS020724mt.zip duplicated into DV-version:
//        P015469: fix problem in correct highlighting in RTFEdit..
//
//
...
//
//
// $Revision: 1.9 $ ----------- 1 Jul 2002
// --RJ: P014696 : EQFBWordDel / EQFBFuncWOrdMark, EQFBFuncCaps:
// use EQFBFindWord instead of EQFBWordPos to find current cursor word
//
//
// $Revision: 1.8 $ ----------- 20 Feb 2002
// --KBT1189: adjust cursor column for Non-Unicode DBCS
//
//
// $Revision: 1.7 $ ----------- 6 Feb 2002
// RJ: fix problem with :NONE tag and allow deletion of complete marked area
//
//
// $Revision: 1.6 $ ----------- 17 Dec 2001
// RJ: get rid of isdbcs1 function
//
//
// $Revision: 1.5 $ ----------- 19 Nov 2001
// RJ: fix problem in correct word recognition if word start with German umlaut
//
//
// $Revision: 1.4 $ ----------- 12 Nov 2001
// RJ: get rid of compiler warnings; fix error ifndef R004353_MARKMORETHAN1SEG
//
//
// $Revision: 1.3 $ ----------- 6 Nov 2001
// RJ: fix problem in DBCS selection under Unicode
//
//
...
// -- RJ: fix mark anomaly if we toggle marking direction (found by Gaby Pelleg)
//
//
// $Revision: 1.4 $ ----------- 17 Apr 2001
// -- RJ: KBT01034: support special marking functions in RTF edit window
//
//
...
//
/*
 * $Header:   K:\DATA\EQFBMARK.CV_   1.18   15 Dec 1998 12:23:50   BUILD  $
 *
 * $Log:   K:\DATA\EQFBMARK.CV_  $
 *
 *    Rev 1.18   15 Dec 1998 12:23:50   BUILD
 * -- KBT0414: allow EQFBFuncMarkDelete if cursor outside but mark inside
 *             active segment
 *
 *    Rev 1.17   29 Jun 1998 16:12:02   BUILD
 * -- KBT00310: FuncMarkNextWord / FuncMarkPrevWord fix
 *
 *    Rev 1.16   22 Jun 1998 16:35:54   BUILD
 * --R004353_MARKMORETHAN1SEG: adjust EQFBFuncMarkBlock, EQFBFuncMarkDelete,
 *   EQFBFuncSegMarkBlock, EQFBFuncWordMark, EQFBFuncMarkCopyMove
 *
 *    Rev 1.15   23 Mar 1998 10:20:18   BUILD
 * -- KBT0260: Ctrl-Right (FuncNextWord) must recognize softLF
 *
 *    Rev 1.14   26 Jan 1998 11:18:36   BUILD
 * -- KBT0138:  fix marking with mouse in DBCS
 * -- KBT0196: donot move one character beyond in case of delete selection
 *
 *    Rev 1.13   19 Jan 1998 10:59:30   BUILD
 * - avoid compiler warnings when compiled for Win32 environment
 *
 *    Rev 1.12   30 Sep 1997 10:43:02   BUILD
 * -- kbt0070: new functions: mark current til start/end of segment
 * -- kbt0070: MarkRight/Left CUA (keyboard): stop marking at segment start/end
 *              MarkUp/MarkDown CUA: don't clear mark if crossing seg boundary
 *
 *    Rev 1.11   15 Sep 1997 10:46:54   BUILD
 * -- KBT0070 part 0: Ctrl-right/Ctrl-left should not position on protected chars
 *
 *    Rev 1.10   25 Feb 1997 13:18:08   BUILD
 * -- Compiler defines for _POE22, _TKT21, and NEWTCSTUFF eliminated
 *
 *    Rev 1.9   11 Nov 1996 09:41:26   BUILD
 * -- corrected extern chDictLookup ..
 *
 *    Rev 1.8   09 Sep 1996 11:08:32   BUILD
 * - KAT0100: Ctrl-Shift-Right stops at line end if !fNoCUASelection:
 *           EQFBMarkNextWord: set fInSelection again after EQFBMarkRightCUA
 * - add support for toggling init character of word
 *
 *    Rev 1.7   15 Apr 1996 10:11:30   BUILD
 * -- fix endless loop in case of dictionary windows and next word due
 *    to a lot of empty blanks at the end and at the beginning...
 *
 *    Rev 1.6   01 Apr 1996 08:33:44   BUILD
 * -- support Shift-Home and Shift-End
 *
 *    Rev 1.5   29 Mar 1996 11:00:16   BUILD
 * -- add EQFBFuncMarkBlock at begin of function EQFBFUncMarkUpCUA and EQFBFuncMarkDownCUA
 * -- EQFBFuncNextWord / PRevWord: goon at line end / line begin
 *
 *    Rev 1.4   28 Mar 1996 20:58:26   BUILD
 * BIDI FLAG REMOVED
 *
 *    Rev 1.3   28 Mar 1996 10:17:58   BUILD
 * -- add EQFBFuncMarkBlock at begin of function EQFBFUncMarkUpCUA and EQFBFuncMarkDownCUA
 *
 *    Rev 1.2   14 Mar 1996 18:40:30   BUILD
 * - BIDI
 *
 *    Rev 1.1   11 Mar 1996 10:29:46   BUILD
 * -- set flag if we are in keyboard selection mode...
 *
 *    Rev 1.0   09 Jan 1996 09:02:06   BUILD
 * Initial revision.
*/
//+----------------------------------------------------------------------------+
#define INCL_BASE
