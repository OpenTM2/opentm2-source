...
#ifndef _EqfSharedMemory_h_
#define _EqfSharedMemory_h_
...
class EqfSharedMemory: public OtmSharedMemory
/*! \brief This class implements the standard shared translation memory (EQF) for OpenTM2.

  The shared memory passes all request to the local memory and sends updates to the shared memory web service.
...
*/
	int initialize( EqfSharedMemoryPlugin *pMemoryPlugin, char *pszMemoryName, OtmMemory *pLocalMem );
...
*/
  ...
  /*! \brief Get plugin responsible for the local copy of this memory
  	  \returns pointer to memory plugin object for local memory
  */
  void *getLocalPlugin();
...
private:
  ...
  EqfSharedMemoryPlugin::PSHAREDMEMPROP pProperties;  // memory properties

/*! \brief proposal related tasks to be performed by web client
*/
	enum eClientTask
	{
		eUpdate,	    		/*!< update an existing propos */
		eDelete,	    		/*!< delete the proposal */
		ePut      	    	/*!< store a new proposal */
	};

/*! \brief Send proposal to the output queue
  	\returns 0 when successful or error code 
*/
  int sendProposalToOutQueue
  ( 
