...
///  USHORT    usOpenAction;    
///
///	pszName = "UserMarkupTablePlugin";
///	pszShortDescription = "User Markup Table Plugin";
///	pszLongDescription = "This is the markup table plugin for the user\'s markup tables";
///	pszVersion = "V1.0";
///	pszSupplier = "User";
///	pluginType = OtmPlugin::eMarkupType;
///	usableState = OtmPlugin::eUsable;
...
///
///
///  strcpy( chTempName, szBasePath );
///  strcat( chTempName, "\\TABLE\\*" ) ;
///  strcat( chTempName, EXT_OF_FORMAT ) ;
///  MarkupObjectList[0] = NULL ;
///  iMarkupCount = 0 ;
///
///  usDosRC = UtlFindFirst( chTempName, &hDirHandle, FILE_NORMAL,
///                       &ResultBuf, sizeof( ResultBuf),
///                       &usCount, 0L, FALSE );
///
///fdaw=fopen("C:\\daw.daw","a");
///fprintf(fdaw,"First:  rc=%d\n",usDosRC);
///if ( usDosRC == 0 ) {
///fprintf(fdaw,"        count=%d\n",usCount);
///if ( usCount>=0 ) {
///fprintf(fdaw,"        name=%s\n",ResultBuf.cFileName);
///}
///}
///fclose(fdaw);
///  int i = 0;
///  while ( usCount && (usDosRC == 0 ) ) 
///  {
///     ptrMarkupInfo = (MARKUPINFO*)malloc( sizeof(MARKUPINFO) ) ;
///     strcpy( chTempName, ResultBuf.cFileName );
///     char *ptr = strrchr( chTempName, '.' );
///     if ( ptr != NULL )
///       *ptr = '\0';
///     ptrMarkupInfo->pszName = (char*)malloc( strlen(chTempName) + 1 ) ;
///     strcpy( ptrMarkupInfo->pszName, chTempName ) ;
///     ptrMarkupInfo->pszShortDescription = (char*)malloc( 5 ) ;
///     strcpy( ptrMarkupInfo->pszShortDescription, "User" ) ;
///     ptrMarkupInfo->pszLongDescription = ptrMarkupInfo->pszShortDescription ;
///     ptrMarkupInfo->pszVersion = (char*)malloc( 4 ) ;
///     strcpy( ptrMarkupInfo->pszVersion, "1.0" );
///     sprintf( chTempName, "TABLE\\%s",ResultBuf.cFileName ) ;
///     ptrMarkupInfo->pszTable = (char*)malloc( strlen(chTempName) + 1 ) ;
///     strcpy( ptrMarkupInfo->pszTable, chTempName ) ;
///     ptrMarkupInfo->pszUserExit = (char*)malloc( 1 ) ;
///     strcpy( ptrMarkupInfo->pszUserExit, "" ) ;
///     ptrMarkupInfo->pszFileList = (char*)malloc( 1 ) ;
///     strcpy( ptrMarkupInfo->pszFileList, "" ) ;
///     MarkupObjectList[i++] = new UserMarkupTable( ptrMarkupInfo, szBasePath );
///     MarkupObjectList[i] = NULL ;
///fdaw=fopen("C:\\daw.daw","a");
///fprintf(fdaw,"ALLOC:  0=%lx  1=%lx  2=%lx  3-%lx  4-%lx  5=%lx  6=%lx  7=%lx\n",
///        ptrMarkupInfo,ptrMarkupInfo->pszName,ptrMarkupInfo->pszShortDescription,ptrMarkupInfo->pszLongDescription,ptrMarkupInfo->pszVersion,ptrMarkupInfo->pszTable,ptrMarkupInfo->pszUserExit,ptrMarkupInfo->pszFileList);
///fclose(fdaw);
///
///     usDosRC = UtlFindNext( hDirHandle, &ResultBuf,
///                         sizeof( ResultBuf), &usCount, FALSE );
///fdaw=fopen("C:\\daw.daw","a");
///fprintf(fdaw,"Next:  rc=%d\n",usDosRC);
///if ( usDosRC == 0 ) {
///fprintf(fdaw,"        count=%d\n",usCount);
///if ( usCount>=0 ) {
///fprintf(fdaw,"        name=%s\n",ResultBuf.cFileName);
...
*/
const bool UserMarkupTablePlugin::updateFiles(
   char   *pszMarkupName,
   char   *pszDescription,
   char   *pszVersion,
