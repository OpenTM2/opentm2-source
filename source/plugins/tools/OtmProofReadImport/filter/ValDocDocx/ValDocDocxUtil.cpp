...
//
//
//  11/29/2004  DAW   P020930. Change x'1A' processing to handle text at beginning of line.   
//   1/26/2005  DAW   P021134. Handle x'0D' in middle of text so no truncation.  
//   3/11/2005  DAW            Add wifstream::get routines for IBMXMWRD.           
//   3/09/2006  DAW            Change RemoveBOM to not allocate more storage.      
//   1/11/2013  DAW   P401634  Handle record lengths at least 20K.
//
//**********************************************************************************************************
...
::wifstream::wifstream( ) {                    /* 1-18-05 for IBMXML */
}

int ::wifstream::open(char *fileName) {        /* 1-18-05 for IBMXML */
    filePtr = fopen(fileName, "rb");
    std::ifstream((char *)fileName);
    return 0 ;
}
    ...
    if ( isUTF16 ) {
        if ( (fgetws(pwch, nCount, this->filePtr) != NULL) ) {
			...
			if ( feof(this->filePtr) ) {
                if ( ( wcslen(pwch) == 0 ) ||           /* 11-29-04 */
                     ( pwch[0] == '\x1A' ) ) 
                   return NULL ;
                if ( pwch[wcslen(pwch)-1] == '\x1A' ) 
                   pwch[wcslen(pwch)-1] = NULL ;
            }
            ptr = wcsrchr(pwch, '\r');         /* wcschr->wcsrchr 1-26-05 */
            if (ptr == NULL)
                ptr = wcsrchr(pwch, '\n');     /* wcschr->wcsrchr 1-26-05 */
       ...
       char    szTemp[MAX_BUFFER_SIZE*3];   /* *2 -> *3   1-11-13 */
...
char* ::wifstream::getline(char *pch, int nCount, BOOL isUTF16) {
    ...
    if( (fgets(pch, nCount, this->filePtr) != NULL) ) {
		...
		ptr = strrchr(pch, '\r');              /* wcschr->wcsrchr 1-26-05 */ 
        if(ptr == NULL )
            ptr = strrchr(pch, '\n');          /* wcschr->wcsrchr 1-26-05 */ 
...
///    this->Replace(pInputString, pOutputString);
    ...
    for( ; count<numChanges ; ++count ) {
       ...
       if ( found < 0 ) break ;
       this->Delete(found,wcslen((WCHAR*)pInputString));
       this->Insert(found,pOutputString);
       start=found+wcslen((WCHAR*)pOutputString);
    }
    return *this;
}
...
int IString::indexOfAnyOf(LPCTSTR pString, int startPos) const {
    ...
    for( ; start<length ; ++start ) {
       if ( wcschr( (WCHAR*)pString, this->GetAt(start) ) )
          return (start+1) ;
    }
    return 0 ;
}
...
IString& IString::overlayWith(const _TCHAR* pOverlayString, int index ) {    
    ...
    int  length2=wcslen((WCHAR*)pOverlayString);
