...
static char *pszShortDescription = "Import proof read documents";
static char *pszLongDescription	= "This is a plugin to import proff reading documents";
...
void OtmProofReadImportPlugin::init()
{
  ...
  manager->addMenuItem( this, "Utilities", "Import proofreading documents...", ID_PROOFREADIMPORT_COMMAND );
...
void OtmProofReadImportPlugin::processCommand( int iCommandID )
{
  if ( iCommandID == ID_PROOFREADIMPORT_COMMAND )
  {
    ...
    if ( fOK ) fOK = getImportFile( strInputFile, strFilterDLL );

    if ( fOK ) saveLastUsedValues( strInputFile, strFilterDLL );

    strProofReadXML = strInputFile;
    strProofReadXML.append( ".proofread.xml" );
    if ( fOK ) fOK = applyFilter( strInputFile, strFilterDLL, strProofReadXML );

    // show the XML file in the proof read window
    if ( fOK ) fOK = showProofReadWindow( strProofReadXML );
  } /* endif */
}
...
*/
void OtmProofReadImportPlugin::fillFilterList()
{
...
*/
BOOL OtmProofReadImportPlugin::getImportFile( std::string &strInputFile, std::string &strFilterDLL )
{
  PSZ pszFileBuffer = new char[MAX_PATH+1]; // buffer for returned file names
  ...
  // split last used file into path and file name
  strcpy( pszInitialDir, strInputFile.c_str() );
  if ( *pszInitialDir )
  {
    PSZ pszFile = UtlSplitFnameFromPath( pszInitialDir );
    strcpy( pszFileBuffer, ( pszFile ) ? pszFile : "" ); 
  }
  else
  {
    *pszFileBuffer = 0;
  ...
  OpenStruct.hwndOwner = (HWND)UtlQueryULong( QL_TWBFRAME );
  OpenStruct.lpstrFilter = pszFilterList;
  ...
  OpenStruct.nMaxFile = MAX_PATH;
  OpenStruct.lpstrFileTitle = NULL;
  OpenStruct.lpfnHook = OpenFileHook;
  OpenStruct.nMaxFileTitle = 0;
  OpenStruct.lpstrTitle = "Import Proofread Document";
  OpenStruct.Flags = OFN_FILEMUSTEXIST | OFN_ENABLEHOOK | OFN_LONGNAMES | OFN_EXPLORER | OFN_NOCHANGEDIR | OFN_PATHMUSTEXIST;

  if ( GetOpenFileName( &OpenStruct ) )
  {
    strFilterDLL = vFilterList[OpenStruct.nFilterIndex-1].szDLLName;
    strInputFile = pszFileBuffer;
    fOK = TRUE;
  }
  else
  {
...
*/
BOOL OtmProofReadImportPlugin::applyFilter( std::string strInputFile, std::string strFilterDLL, std::string strProofReadXML )
{
  BOOL fOK = TRUE;
  ...
  {
    fOK = pConvertFunc( strInputFile.c_str(), strProofReadXML.c_str() ); 
  }
  else
  {
    fOK = FALSE;
...
/*! \brief Show the proofreading process window
	...
	\returns TRUE if the function completed successfully
*/
BOOL OtmProofReadImportPlugin::showProofReadWindow( std::string strProofReadXML )
{
  BOOL fOK = TRUE;
