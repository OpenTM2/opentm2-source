...
CPlginPendLst::CPlginPendLst(void)
{
    memset(m_strCfgPath, 0x00, sizeof(m_strCfgPath));
}

CPlginPendLst::CPlginPendLst(const char * strOtmPath)
{
    ...
    {
        sprintf(m_strCfgPath, "%s%s\\%s\\%s", strOtmPath, OPENTM2_PLUGIN_FOLDER_STR, OPENTM2_PENDING_FOLDER_STR, PLUGIN_PENDING_LST);
    }
    else
    {
        sprintf(m_strCfgPath, "%s\\%s\\%s\\%s", strOtmPath, OPENTM2_PLUGIN_FOLDER_STR, OPENTM2_PENDING_FOLDER_STR, PLUGIN_PENDING_LST);
...
int CPlginPendLst::ParseConfigFile()
{
    int nRC = NO_ERROR;
...
int CPlginPendLst::GetPendingCnt()
{
    int nCnt = 0;
...
const char * CPlginPendLst::GetPendingName(int nInx)
{
...
int CPlginPendLst::SetPendingResult(int nInx, int nRet)
{
    int nRC = NO_ERROR;
...
int CPlginPendLst::RefreshPendingLst()
{
    int nRC = NO_ERROR;
    int nEndCnt = 0;
